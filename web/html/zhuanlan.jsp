...
<script type="text/javascript">
	...
	function goPage(currPage){
		...
		$(".ColumnHomeColumnCard").remove();
		$("[name=curr]").remove();
		$.ajax({
			url:"<%=path%>/Column?name=3",
			type:"post",
			data:{"curr":currPage},
			dataType:"json",
			success:function(result){
				console.log(result);
				$("[name=first]").attr("onclick","goPage(1)");
				$("[name=prev]").attr("onclick","goPage("+result.prev+")");
				$("[name=next]").attr("onclick","goPage("+result.next+")");
				$("[name=last]").attr("onclick","goPage("+result.last+")");

				//循环所有页数，生成页码
				for ( var i = 0; i < result.last; i++) {
					var $cnstr = $('<button id="btn" value="'+(i+1)+'" name="curr" onclick="goPage('+(i+1)+')" type="button" class="Button ColumnHomeRecommendation-refreshButton Button--grey Button--withIcon Button--withLabel">\n'+
							''+(i+1)+'\n'+
							'</button>');

					$("[name=next]").before($cnstr);



				}
				/* var num = document.getElementById("btn").getAttribute("value");
                   $("[name=curr]").attr("onclick","goPage("+num+")"); */

				$.each(result.cnList,function(i,obj){
					var $allcn = $('<a href="<%=path%>/ColumnById?cnId='+obj.id+'" class="ColumnHomeColumnCard ColumnHomeRecommendation-Card" target="_blank"></a>');
					var item = $('<img class="Avatar Avatar--medium Avatar--round ColumnHomeColumnCard-Avatar" width="48" height="48" src="images/zhuanlan/'+obj.columnLOGO+'" alt="头像专栏" />\n'+
							'<div class="ColumnHomeColumnCard-info">\n'+
							'<div class="ColumnHomeColumnCard-title">'+obj.columnTItle+'</div>\n'+
							'<div class="ColumnHomeColumnCard-description">\n'+
							''+obj.columnSlogan+'\n'+
							'</div>\n'+
							'</div>\n'+
							'<div class="ColumnHomeColumnCard-meta">'+obj.followNum+'人关注 |1，058篇文章</div>\n'+
							'<button type="button" class="Button ColumnHomeColumnCard-followButton Button--green">进入专栏</button>');
					$allcn.append(item);
					$(".ColumnHomeRecommendation-cardContainer").append($allcn);
				})
	...
	$.ajax({
		...
		success:function(result){
			...
			/* $.each(result,function(i,obj){
                var $allcn = $('<a class="ColumnHomeColumnCard ColumnHomeRecommendation-Card" target="_blank"></a>');
                var item = $('<img class="Avatar Avatar--medium Avatar--round ColumnHomeColumnCard-Avatar" width="48" height="48" src="images/zhuanlan/'+obj.columnLOGO+'" alt="头像专栏" />\n'+
                          '<div class="ColumnHomeColumnCard-info">\n'+
                              '<div class="ColumnHomeColumnCard-title">'+obj.columnTItle+'</div>\n'+
                              '<div class="ColumnHomeColumnCard-description">\n'+
			...
			var $item = $('<div class="butDiv"></div>');
			var items = $('<button name="first" type="button" class="Button ColumnHomeRecommendation-refreshButton Button--grey Button--withIcon Button--withLabel">\n'+
					'首页\n'+
					'</button>\n'+
					'<button name="prev" type="button" class="Button ColumnHomeRecommendation-refreshButton Button--grey Button--withIcon Button--withLabel">\n'+
					'上一页\n'+
					'</button>\n'+
					'<button name="next" type="button" class="Button ColumnHomeRecommendation-refreshButton Button--grey Button--withIcon Button--withLabel">\n'+
					'下一页\n'+
					'</button>\n'+
					'<button name="last" type="button" class="Button ColumnHomeRecommendation-refreshButton Button--grey Button--withIcon Button--withLabel">\n'+
					'尾页\n'+
					'</button>');
			$item.append(items);
			$(".ColumnHomeRecommendation-cardContainer").after($item);
